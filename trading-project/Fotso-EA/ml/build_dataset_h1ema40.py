...
def compute_signal_score(direction, h1_bull, h1_bear,
                          ...
                          adx_min=22.0):
    """Réplique EvaluerCerveau() EA v6.1 avec direction H1 EMA40."""
    # Direction gate : H1 EMA40 (v6.1)
    if direction and not h1_bull:
        return 0, 0, False, False, False
    if not direction and not h1_bear:
        return 0, 0, False, False, False
    if adn_phase == 0:
        return 0, 0, False, False, False
    if adn_phase == 1:
...
def simulate_outcome(m15, entry_idx, direction, sl, tp1, tp2, max_bars=200):
    n = len(m15)
    ...
    for k in range(1, min(max_bars, n - entry_idx)):
        ...
        if direction:
            if hi >= tp1 and not tp1_hit: label_tp1 = 1; tp1_hit = True
            if hi >= tp2 and tp1_hit:     label_tp2 = 1; bars_out = k; break
            if lo <= sl:                  bars_out = k; break
        else:
            if lo <= tp1 and not tp1_hit: label_tp1 = 1; tp1_hit = True
            if lo <= tp2 and tp1_hit:     label_tp2 = 1; bars_out = k; break
...
def build_dataset(data_dir: str, out_path: str, cfg: dict) -> pd.DataFrame:
    ...
    for i in range(warmup, n_m15 - max_fwd - 1):
        ...
        for direction in [True, False]:  # True=BUY, False=SELL
            # Block SELL si H1 EMA40 ultra haussier (EA v6.1)
            ...
            lbl_tp1, lbl_tp2, bars_out = simulate_outcome(m15, i, direction, sl, tp1, tp2, max_fwd)
