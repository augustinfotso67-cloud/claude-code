...
#property strict
//+------------------------------------------------------------------+
//|  CHANGELOG v5.92 — INTERRUPTEUR DE REGIME (Regime Gate A∧B)      |
//|                                                                  |
//|  AUTEUR DU PATCH : Mahamat (FOTSO COMPANY, genie informatique)   |
//|  SPEC : Louis (quant)                                            |
//|                                                                  |
//|  PROBLEME : v5.91 est trend-following long-biased XAU M15.       |
//|    - GAGNE en bull directionnel (2025)                           |
//|    - PERD en chop haut-niveau (2026) et en bear (2022)           |
//|                                                                  |
//|  HYPOTHESE DE LOUIS : l'ADX D1 separe le bull DIRECTIONNEL       |
//|    (ou l'EA gagne) du chop HAUT-NIVEAU (ou l'EA se fait hacher). |
//|    Un prix au-dessus de l'EMA200 D1 avec pente positive ET un    |
//|    ADX D1 eleve = le seul regime ou l'edge de l'EA existe.       |
//|                                                                  |
//|  SOLUTION : un interrupteur de regime a 2 filtres (conjonction   |
//|  stricte A ET B) qui n'autorise le trade QUE dans ce regime :    |
//|    FILTRE A (tendance D1) :                                      |
//|       - Close D1 [1] > EMA200 D1 [1]                             |
//|       - pente positive : EMA200_D1[1] > EMA200_D1[SlopeBars+1]   |
//|    FILTRE B (regime/directionnalite D1) :                        |
//|       - ADX(period) D1 [1] >= InpRegimeADX_Min                   |
//|                                                                  |
//|  ANTI LOOK-AHEAD : tous les indicateurs D1 sont lus sur barre    |
//|    FERMEE [1], jamais [0] en cours de formation.                 |
//|                                                                  |
//|  BRANCHEMENT : gate evalue en tete de OuvrirTrade() — couvre les |
//|    4 chemins d'ouverture (BUY/SELL tendance + BUY/SELL pullback) |
//|    en un seul point. Si InpUseRegimeGate=false => comportement   |
//|    STRICTEMENT identique a v5.91 (aucun chemin modifie sinon).   |
//|                                                                  |
//|  HANDLES : 2 handles D1 dedies (EMA200 + ADX) crees une seule    |
//|    fois dans OnInit, liberes dans OnDeinit. PAS de creation/tick.|
//+------------------------------------------------------------------+
...
// ── v5.92 : INTERRUPTEUR DE REGIME (Regime Gate A∧B) — spec Louis ──
input bool   InpUseRegimeGate   = true;   // master switch de l'interrupteur de regime
input int    InpRegimeADX_Min   = 25;     // ADX D1 minimum (Louis : tester 22/25/28)
input int    InpRegimeADX_Period= 14;     // periode ADX D1
input int    InpRegimeEMA_Period= 200;    // EMA D1 pour le filtre tendance
input int    InpRegimeSlopeBars = 20;     // barres pour la pente EMA200 D1
...
// ── v5.92 : handles D1 dedies a l'interrupteur de regime ──────────
// Crees une seule fois dans OnInit (InitIndicateurs), liberes OnDeinit.
// Separes des handles existants pour ne RIEN changer au comportement
// quand InpUseRegimeGate=false.
int hRegimeEMA_D1 = INVALID_HANDLE;   // EMA(InpRegimeEMA_Period) D1 pour filtre A
int hRegimeADX_D1 = INVALID_HANDLE;   // ADX(InpRegimeADX_Period) D1 pour filtre B
...
void OnDeinit(const int reason)
  {
   ...
   // v5.92 : liberation des handles D1 du Regime Gate (si crees)
   if(hRegimeEMA_D1 != INVALID_HANDLE) IndicatorRelease(hRegimeEMA_D1);
   if(hRegimeADX_D1 != INVALID_HANDLE) IndicatorRelease(hRegimeADX_D1);
...
//|   v5.92 — INTERRUPTEUR DE REGIME (Regime Gate A∧B) — spec Louis  |
//|                                                                  |
//|   Retourne TRUE (trade autorise) seulement si A==true ET B==true.|
//|                                                                  |
//|   FILTRE A (tendance D1) :                                       |
//|     - Close D1 [1] > EMA200 D1 [1]                               |
//|     - pente positive : EMA200_D1[1] > EMA200_D1[SlopeBars+1]     |
...
//|     - ADX(period) D1 [1] >= InpRegimeADX_Min                     |
//|                                                                  |
//|   ANTI LOOK-AHEAD : toute lecture D1 sur barre FERMEE [1].       |
//|   On copie assez de barres pour atteindre l'index SlopeBars+1,   |
//|   en series-time (index 0 = barre courante non close).          |
//+------------------------------------------------------------------+
bool PassRegimeGate()
  {
   // Si le gate est desactive, on n'altere rien : v5.91 a l'identique.
   if(!InpUseRegimeGate) return true;

   if(hRegimeEMA_D1 == INVALID_HANDLE || hRegimeADX_D1 == INVALID_HANDLE)
     {
      // Handles absents (ex: gate active a chaud sans re-init) : on bloque
      // par prudence plutot que de trader hors-regime.
      Print("REGIME GATE: handles D1 invalides — trade bloque (securite)");
      return false;
     }

   // Nombre de barres EMA a copier : il faut pouvoir lire l'index
   // InpRegimeSlopeBars+1. +2 de marge pour la robustesse du CopyBuffer.
   int needEma = InpRegimeSlopeBars + 3;

   double emaD1[]; ArraySetAsSeries(emaD1, true);
   double adxD1[]; ArraySetAsSeries(adxD1, true);

   // EMA200 D1 : on lit [1] (barre close) et [SlopeBars+1] (pente).
   int gotEma = CopyBuffer(hRegimeEMA_D1, 0, 0, needEma, emaD1);
   if(gotEma < needEma)
     {
      Print("REGIME GATE: CopyBuffer EMA D1 insuffisant (", gotEma,
            "/", needEma, ") — trade bloque");
      return false;
     }

   // ADX D1 : on lit [1] (barre close). 3 barres suffisent.
   int gotAdx = CopyBuffer(hRegimeADX_D1, 0, 0, 3, adxD1);
   if(gotAdx < 3)
     {
      Print("REGIME GATE: CopyBuffer ADX D1 insuffisant (", gotAdx,
            "/3) — trade bloque");
      return false;
     }

   // Close D1 de la barre FERMEE [1].
   double closeD1_1 = iClose(Symbol(), PERIOD_D1, 1);
   if(closeD1_1 <= 0)
     {
      Print("REGIME GATE: Close D1 [1] indisponible — trade bloque");
      return false;
     }

   double emaD1_1     = emaD1[1];                       // EMA200 D1 barre close
   double emaD1_slope = emaD1[InpRegimeSlopeBars + 1];  // EMA200 D1 il y a SlopeBars barres
   double adxD1_1     = adxD1[1];                       // ADX D1 barre close

   // ── FILTRE A : tendance D1 ──────────────────────────────────────
   bool aClose = (closeD1_1 > emaD1_1);
   bool aSlope = (emaD1_1 > emaD1_slope);
   bool A      = (aClose && aSlope);

   // ── FILTRE B : regime/directionnalite D1 ────────────────────────
   bool B = (adxD1_1 >= (double)InpRegimeADX_Min);

   // ── CONJONCTION STRICTE A ET B ──────────────────────────────────
   bool pass = (A && B);

   if(!pass)
      Print("REGIME GATE: trade bloque (A=", A, " B=", B,
            " ADX=", DoubleToString(adxD1_1, 1), ")");
...
void OuvrirTrade(bool isBuy, ADNMouvement &adn, MiroirInstitutionnel &mi,
  ...
  {
   // ── v5.92 : INTERRUPTEUR DE REGIME (point de branchement unique) ─
   // Couvre les 4 chemins d'ouverture (BUY/SELL tendance + pullback).
   // Si gate off => PassRegimeGate() retourne true => v5.91 identique.
   if(InpUseRegimeGate && !PassRegimeGate())
      return;  // Hors regime viable : pas de trade.
...
bool InitIndicateurs()
  {
   ...
   // ── v5.92 : handles D1 de l'interrupteur de regime ──────────────
   // Crees uniquement si le gate est actif : aucun cout/effet si off.
   if(InpUseRegimeGate)
     {
      hRegimeEMA_D1 = iMA(Symbol(), PERIOD_D1, InpRegimeEMA_Period, 0,
                         MODE_EMA, PRICE_CLOSE);
      hRegimeADX_D1 = iADX(Symbol(), PERIOD_D1, InpRegimeADX_Period);
      if(hRegimeEMA_D1 == INVALID_HANDLE || hRegimeADX_D1 == INVALID_HANDLE)
        {
         Alert("Regime Gate v5.92 : echec creation handles D1 — "
               "verifier historique D1 !");
         return false;
        }
     }
   return true;
